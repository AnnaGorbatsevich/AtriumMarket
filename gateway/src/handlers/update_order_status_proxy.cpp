#include "update_order_status_proxy.hpp"
...
constexpr std::string_view kRequiredFields[] = {"orderId", "status"};
...
UpdateOrderStatusProxyHandler::UpdateOrderStatusProxyHandler(
...
std::string UpdateOrderStatusProxyHandler::HandleRequestThrow(
    ...
    forwarded_payload["orderId"] = payload["orderId"];
    forwarded_payload["status"] = payload["status"];
    forwarded_payload["actorId"] = identity["id"].As<std::int64_t>();
    forwarded_payload["actorRole"] = identity["role"].As<std::string>();
    ...
    try {
        ...
        if (upstream_response->status_code() == 200) {
            ...
            if (body["newStatus"].As<std::string>({}) == "cancelled") {
                try {
                    http_requests_.DecreaseAvailability(

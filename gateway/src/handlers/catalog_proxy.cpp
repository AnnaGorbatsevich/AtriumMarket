...
CatalogProxyHandler::CatalogProxyHandler(
...
std::string CatalogProxyHandler::HandleRequestThrow(
    ...
    auto url = ListingServiceUrl() + "/catalog";
    if (request.HasArg("categoryId") && !request.GetArg("categoryId").empty()) {
        url += "?categoryId=" + request.GetArg("categoryId");
    }

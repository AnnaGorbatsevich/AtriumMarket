...
CategoriesProxyHandler::CategoriesProxyHandler(
...
std::string CategoriesProxyHandler::HandleRequestThrow(
    ...
    try {
        auto upstream_response = http_client_.CreateRequest()
                                      .get(ListingServiceUrl() + "/categories")

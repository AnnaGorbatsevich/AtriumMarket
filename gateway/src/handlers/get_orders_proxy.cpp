...
std::string GetOrdersProxyHandler::HandleRequestThrow(
    ...
    const bool has_seller_id = request.HasArg("sellerId") && !request.GetArg("sellerId").empty();
    const bool has_buyer_id = request.HasArg("buyerId") && !request.GetArg("buyerId").empty();

    if (!has_seller_id && !has_buyer_id) {
        throw userver::server::handlers::ClientError(
            userver::server::handlers::ExternalBody{"Missing required query parameter: sellerId or buyerId"}
        );
    }

    auto url = OrderServiceUrl() + "/get_orders?";
    url += has_seller_id ? "sellerId=" + request.GetArg("sellerId") : "buyerId=" + request.GetArg("buyerId");

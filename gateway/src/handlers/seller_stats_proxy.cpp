#include "seller_stats_proxy.hpp"
...
SellerStatsProxyHandler::SellerStatsProxyHandler(
...
std::string SellerStatsProxyHandler::HandleRequestThrow(
    ...
    if (identity["role"].As<std::string>({}) != "seller") {
        throw userver::server::handlers::CustomHandlerException(
            ...
            userver::server::handlers::ExternalBody{"Only sellers can view their statistics"}
        );
    }
    ...
    try {
        auto upstream_response = http_requests_.Get(
            StatsServiceUrl(), "/seller_stats?sellerId=" + std::to_string(seller_id), {}, 2000
        );
        ...
        throw userver::server::handlers::CustomHandlerException(
            userver::server::handlers::HandlerErrorCode::kBadGateway,
            userver::server::handlers::ExternalBody{"stats-service is unavailable"}
        );
    }
}
